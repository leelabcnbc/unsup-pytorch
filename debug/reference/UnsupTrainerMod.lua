...
local UnsupTrainer = torch.class('unsup.UnsupTrainerMod')
...
function UnsupTrainer:train(params)
   ...
   -- save all
   npy4th.savenpy('demo_psd_conv_debug_data.npy', torch.cat(all_data,1))
   npy4th.savenpy('demo_psd_conv_debug_weight.npy', torch.cat(all_weight,1))
    
   npy4th.savenpy('demo_psd_conv_debug_encoder_weight.npy', torch.cat(all_encoder_weight,1))
   npy4th.savenpy('demo_psd_conv_debug_encoder_bias.npy', torch.cat(all_encoder_bias,1))
   npy4th.savenpy('demo_psd_conv_debug_encoder_scale.npy', torch.cat(all_encoder_scale,1))
    
   npy4th.savenpy('demo_psd_conv_debug_grad_weight.npy', torch.cat(all_grad_weight,1))
   npy4th.savenpy('demo_psd_conv_debug_serr.npy', torch.FloatTensor(all_serr))
end
